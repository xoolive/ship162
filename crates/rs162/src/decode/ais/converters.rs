...
pub fn from_turn(raw: u8) -> f32 {
    let signed_val = raw as i8;

    match signed_val {
        -128 => super::common::TurnRate::NO_TI_DEFAULT,
        -127 => 708.0,               // turning right at more than 5°/30s
        127 => -708.0,               // turning left at more than 5°/30s
        val => (val as f32) * 4.733, // Convert to degrees per minute
    }
}

/// Convert speed from raw 10-bit value to knots
pub fn from_speed(raw: u16) -> f32 {
    match raw {
        1023 => 102.3, // Not available
        1022 => 102.2, // 102.2 knots or higher
        val => val as f32 / 10.0,
    }
}

/// Convert longitude from raw 28-bit signed value to degrees
pub fn from_longitude(raw: u32) -> f64 {
    // Convert to signed 28-bit value
    ...
    if signed_val == 0x6791AC0 {
        ...
        signed_val as f64 / 600000.0
    }
}

/// Convert latitude from raw 27-bit signed value to degrees  
pub fn from_latitude(raw: u32) -> f64 {
    // Convert to signed 27-bit value
...
pub fn from_course(raw: u16) -> f32 {
    match raw {
        3600 => 360.0, // Not available
        val => val as f32 / 10.0,
    }
}

/// Convert draught from raw 8-bit value (1/10 meters)
pub fn from_10th_u8(raw: u8) -> f32 {
    match raw {
        255 => 25.5, // Not available
        val => val as f32 / 10.0,
    }
}
...
pub fn from_longitude_600(raw: u32) -> f64 {
    if raw == 0x3FFFF {
        // Not available
        0.0
    } else {
...
pub fn from_latitude_600(raw: u32) -> f64 {
    if raw == 0x1FFFF {
        // Not available
        0.0
    } else {
...
mod tests {
    use super::*;
    use crate::decode::ais::common::*;

    #[test]
    fn test_from_turn() {
        assert_eq!(from_turn(0x80), TurnRate::NO_TI_DEFAULT); // -128
        assert_eq!(from_turn(0), 0.0);
        assert_eq!(from_turn(1), 4.733);
        assert_eq!(from_turn(0xFF), -4.733); // -1 as i8
    }

    #[test]
    fn test_from_speed() {
        assert_eq!(from_speed(0), 0.0);
        assert_eq!(from_speed(100), 10.0);
        assert_eq!(from_speed(1023), 102.3);
    }

    #[test]
    fn test_from_longitude() {
        assert_eq!(from_longitude(0), 0.0);
        assert_eq!(from_longitude(0x6791AC0), 181.0); // Not available

        // Test positive longitude (1 degree = 600000 raw units)
        assert!((from_longitude(600000) - 1.0).abs() < 0.000001);
        ...
        assert!((from_longitude(neg_one_raw) - (-1.0)).abs() < 0.000001);
    }

    #[test]
    fn test_from_latitude() {
        assert_eq!(from_latitude(0), 0.0);
        assert_eq!(from_latitude(0x3412140), 91.0); // Not available

        // Test positive latitude (1 degree = 600000 raw units)
        assert!((from_latitude(600000) - 1.0).abs() < 0.000001);
        ...
        assert!((from_latitude(neg_one_raw) - (-1.0)).abs() < 0.000001);
    }

    #[test]
    fn test_from_course() {
        assert_eq!(from_course(0), 0.0);
        assert_eq!(from_course(900), 90.0);
        assert_eq!(from_course(3600), 360.0); // Not available
    }

    #[test]
    fn test_from_10th_u8() {
        assert_eq!(from_10th_u8(0), 0.0);
        assert_eq!(from_10th_u8(122), 12.2);
        assert_eq!(from_10th_u8(255), 25.5);
    }
